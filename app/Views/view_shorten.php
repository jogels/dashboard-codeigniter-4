...
foreach ($SummPROJ as $row) {
    ...
    if ($row['PROJ_Field_start'] != '0000-00-00') {
        ...
        if ($row['PROJ_Status_DP'] == 0 && $row['PROJ_StatusFieldFinal'] == 0) {
            ...
            $bantu['SummQC'] = $id3;
            $bantu['SummNWB'] = $id5;
            $bantu['CekStartFW'] = datedifference($row['PROJ_Field_start'], $skarang)+1;
            $index = 0;
            if((datedifference($row['PROJ_Field_start'], $skarang))<0) {$bantu['Status'] = '1. DP:Scripting';}
            if((datedifference($row['PROJ_Field_start'], $skarang))>=0&&(datedifference($row['PROJ_Field_End'], $skarang))<=0) {$bantu['Status'] = '2. FW Stage';}
            if((datedifference($row['PROJ_Field_End'], $skarang))>0) {$bantu['Status'] = '3. DP:Tabulation';}
            $lfw = datedifference($row['PROJ_Field_start'], $row['PROJ_Field_End']);
            $lnowtoend = datedifference($skarang, $row['PROJ_Field_End']);
            $lstarttonow = datedifference($row['PROJ_Field_start'], $skarang)+1;
...
//array_sort($arr_dashboard, 'INDEXPCT', SORT_ASC);
$columns = array_column($arr_dashboard, "PROF_NO");
...
<div id="div_refresh">
    ...
    <div class="main-content">
        <section class="section">
            ...
            <?php $session = session() ?>
            <h4>Selamat datang <?php echo $session->get('username') ?> (Anda sebagai User)  - <strong>LIST OF CLOSED PROJECTS - LAST 20 PROJECTS</strong></h4> 
