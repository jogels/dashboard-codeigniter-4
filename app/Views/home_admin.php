...
while ($row = mysqli_fetch_assoc($bantu01)) {
    ...
    $selisihwaktu=datedifference($skarang,$row['PROJ_Field_End_Adj']);
    // if($id!=''&&strpos($row['PROJ_name_parent'],'Smoothie QT ')==''){
    if($id!=''){
        if(($selisihwaktu>=0)||($ar_ach[$id]['SUMACH']>0&&$selisihwaktu<0&&$ar_ach[$id]['SUMACH']<=$row['Sum_Target'])){
...
foreach ($SummPROJ as $row) {
    ...
    // if ($row['PROJ_Field_start'] != '0000-00-00' ) {
        ...
        if ($row['PROJ_StatusFieldFinal'] == 1 ) {
            ...
            if(((datedifference($row['PROJ_Field_start'], $skarang))+0)>0&&((datedifference($row['PROJ_Field_End_Adj'], $skarang))+0)<=0) {$bantu['Status'] = '2. FW Stage';}
            if(((datedifference($row['PROJ_Field_End_Adj'], $skarang))+0)>0) {$bantu['Status'] = '3. DP:Tabulation';}
            ...
            $bantu['DailyRemain'] = '';
            $bantu['now'] = $skarang;
            if ($row['PROJ_Field_start'] != '0000-00-00') {$bantu['PCT_TIME'] = round($lstarttonow / $row['lamafw'] * 100, 0);} {$bantu['PCT_TIME'] ='-';}
            ...
            if ($row['PROJ_Field_start'] != '0000-00-00') {$rasiotime = $lstarttonow /  $row['lamafw'];} {$rasiotime ='-';}
                         ...
                         <table border="1" cellpadding="3" cellspacing="0" width=100% style='border-collapse:collapse; table-layout:absolute ; font-size:11px'>
                            <thead>
                                <tr height=20 style="font-size:15px" ; bgcolor="dodgerblue">
                                    ...
                                    <th rowspan=2 class=xl124 width=80 style='width:80px'>
                                        <font color="white">FW-End (FBN)</font>
                                    </th>
                                    <th rowspan=2 class=xl124 width=80 style='width:80px'>
                                        <font color="white">FW-End (*adj)</font>
                                    </th>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr style="font-size:15px">
                                        ...
                                        <td bgcolor="white"><a href="/bydate?NWB_INT=<?= $row['NWB_ID'] . ';' . $row['NWB_ACH']; ?>"><font color="black"><?= $row['PROJ_Field_End_Adj']; ?></font></a></td>
