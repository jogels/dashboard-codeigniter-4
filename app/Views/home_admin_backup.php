...
////==========final 
$newarray = array_column($ar_ach, 'NWB_ID');
$strquery = "SELECT *,AVG(PROJ_Target) as Sum_Target, MIN(PROJ_Field_start) as minPROJ_Field_start,MAX(PROJ_Field_End) as maxPROJ_Field_End FROM tb_Project_ownership WHERE PROJ_ID10DGT Not like '2204144301' and (PROJ_Status_DP=1 or PROJ_StatusFieldFinal = 1) GROUP BY PROJ_KODENWB";
$bantu01 = mysqli_query($conn, $strquery);
$SummPROJ = [];
$row = [];
$bantu = [];
while ($row = mysqli_fetch_assoc($bantu01)) {
    ...
    if($id!=''){
        if((datedifference($skarang,$row['PROJ_Field_End'])>=0)||$ar_ach[$id]['SUMACH']<=$row['Sum_Target']){
...
//####################EDIT Buat SMOOTHIE##################
$newarray = array_column($ar_ach, 'NWB_ID');
$strquery = "SELECT *,SUM(PROJ_Target) as Sum_Target, MIN(PROJ_Field_start) as minPROJ_Field_start,MAX(PROJ_Field_End) as maxPROJ_Field_End FROM tb_Project_ownership WHERE PROJ_ID10DGT LIKE '2204144301' and (PROJ_Status_DP=1 or PROJ_StatusFieldFinal = 1) GROUP BY PROJ_ID10DGT";
$bantu01 = mysqli_query($conn, $strquery);
// $SummPROJ_Smoothie = [];
while ($row = mysqli_fetch_assoc($bantu01)) {
    $bantu=[];
