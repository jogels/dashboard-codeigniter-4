...
$strquery = "SELECT PROJ_Project_name,NWB_12digit,NWB_INT,SUM(IF(NWB_STATUS = 'Complete',NWB_Num,0)) AS vValid,SUM(IF(NWB_STATUS = 'Cancelled',NWB_Num,0)) AS vCancel FROM tb_NWB LEFT JOIN tb_Project_ownership ON NWB_ID = PROJ_KODENWB  WHERE PROJ_StatusFieldFinal=1  GROUP BY NWB_INT";
$bantu01 = mysqli_query($conn, $strquery);
$bantuall = [];
while ($row = mysqli_fetch_assoc($bantu01)) {
    $bantuall[] = $row;
...
foreach ($bantuall as $row) {
    // if($row['NWB_12digit']!=''){
        ...
        // $bantu['NWB_12digit']=$row['NWB_12digit'];
        $bantu['NWB_INT']=$row['NWB_INT'];
        $bantu['sum_ach']=$row['vValid'];
        $bantu['sum_cancel']=$row['vCancel'];
...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            ...
            <h1>RESUME RTD <br>
                <strong>
                    <font color="darkblue"><?= 'LOAD INTERVIEWER (VENDOR)' ?></font>
                </strong></br>
                </h4><br><br>
                ...
                <div class="card-body col-md-8">
                    ...
                    <table border="1" cellpadding="3" cellspacing="0" width=100% style='border-collapse:collapse; table-layout:absolute ; font-size:11px'>
                        ...
                        <tbody>
                            				    ...
                            				    if($row['RTD_IDVENDOR']!='11111111' || $row['RTD_IDVENDOR']=='') {
                                				   $i=$i+1 ?> 
