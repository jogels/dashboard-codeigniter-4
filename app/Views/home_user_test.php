...
foreach ($SummPROJ as $row) {
    ...
    if ($row['PROJ_Field_start'] != '0000-00-00') {
        if ($row['PROJ_StatusFieldFinal'] == 1 && (datedifference($row['PROJ_Field_start'], $skarang))>=0) {
            ...
            $bantu['PROJ_SFM'] = $row['PROJ_SFM'];
            if(!empty($id4)){$bantu['NWB_ID'] = $arr_varian[$id4]['NWB_ID'];}else{$bantu['NWB_ID']='-';}
...
<div id="div_refresh">
    ...
    <div class="main-content">
        <section class="section">
            ...
            <?php $session = session() ?>
            <h4>Selamat datang <?php echo $session->get('username') ?> (Anda sebagai User) </h4>
            <!--<h4>Selamat datang User, <?php echo $session->get('username') ?></h4>-->
                        ...
                        <table border="1" cellpadding="3" cellspacing="0" width=100% style='border-collapse:collapse; table-layout:absolute ; font-size:11px'>
                            <thead>
                                <tr height=20 style="font-size:15px" ; bgcolor="dodgerblue">
                                    ...
                                    <th rowspan=2 class=xl132 width=150 style='width:60px'>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($arr_dashboard as $row) { ?>
                                    <tr style="font-size:15px">
                                        ...
                                        <td><a href="/db_quota?NWB_INT=<?= $row['NWB_10digit'] . ';' . $row['PROJ_NAMEORIGINAL'] . ';' . $row['NWB_ACH']. ';' . $row['PROJ_NAME']; ?>"><?= $row['NWB_ACH']; ?></td>
                                        <!--<td><a href="/db_quota?NWB_INT=<?= $row['NWB_10digit'] . ';' . $row['PROJ_NAMEORIGINAL'] . ';' . $row['NWB_ACH']; ?>"><?= $row['NWB_ACH']; ?></td>-->
