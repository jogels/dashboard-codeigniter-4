...
////==========final 
$strquery = "SELECT *,AVG(PROJ_Target) as Sum_Target, MIN(PROJ_Field_start) as minPROJ_Field_start,MAX(PROJ_Field_End) as maxPROJ_Field_End,MAX(PROJ_Field_End_Adj) as maxPROJ_Field_End_Adj  FROM tb_Project_ownership WHERE PROJ_Status_DP=1 or PROJ_StatusFieldFinal = 1 GROUP BY PROJ_Symphoni_num,PROJ_KODENWB";
$bantu01 = mysqli_query($conn, $strquery);
$SummPROJ = [];
$row = [];
$bantu = [];
while ($row = mysqli_fetch_assoc($bantu01)) {
    ...
    $bantu['PROJ_Field_End_Adj'] = $row['maxPROJ_Field_End_Adj'];
...
foreach ($SummPROJ as $row) {
    ...
    // if ($row['PROJ_Field_start'] != '0000-00-00') {
        ...
        if ($row['PROJ_Status_DP'] == 1 || $row['PROJ_StatusFieldFinal'] == 1) {
            $bantu = [];
            $bantu['SummPROJ'] = $id;
...
//array_sort($arr_dashboard, 'INDEXPCT', SORT_ASC);
$columns = array_column($arr_dashboard, "idsort");
...
<div id="div_refresh">
    ...
    <div class="main-content">
        <section class="section">
            ...
            <?php $session = session() ?>
            <h4>Selamat datang <?php echo $session->get('username') ?> (Anda sebagai Admin) - <strong>LIST OF ACTIVE PROJECTS - TABULATION</strong></h4>
                         ...
                         <table border="1" cellpadding="3" cellspacing="0" width=100% style='border-collapse:collapse; table-layout:absolute ; font-size:11px'>
                            <thead>
                                <tr height=20 style="font-size:15px" ; bgcolor="dodgerblue">
                                    ...
                                    <th rowspan=2 class=xl124 width=80 style='width:80px'>
                                        <font color="white">FW-End (*Adjusted)</font>
                                    </th>
                            ...
                            <tbody>
                                ...
                                if((strtolower(trim(substr($row['Status'], 3, 50)))=='dp:tabulation') || ($row['PROJ_StatusFieldFinal']==0&&$row['PROJ_Status_DP']==1)){ 
                                ?>
