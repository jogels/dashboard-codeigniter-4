...
$str1 = "SELECT NWB_ID, NWB_10digit,NWB_12digit,NWB_IDPROJ,TimeDownload,COUNT(NWB_Num) as COUNTACH,SUM(NWB_Num) as SUMACH ";
$str2 = " FROM tb_NWB WHERE NWB_STATUS LIKE 'Complete' and NWB_10digit Not Like '2204144301'  GROUP BY NWB_ID";
$strqry = $str1 . " " . $str2;
$bantu01 = mysqli_query($conn, $strqry);
$bantu = [];
$row = [];
$ar_ach = [];
while ($row = mysqli_fetch_assoc($bantu01)) {
    // if(strpos($row['PROJ_name_parent'],'Smoothie QT ')==''){
    $bantu[] = $row;
    $ar_ach[] = $row;
    // }
}
//############### EDIT ACHIEVEMENT ####################
$newarray = array_column($ar_ach, 'NWB_ID');
$id = array_search('S22025383', $newarray, true);
$ar_ach[$id]['SUMACH'] = $ar_ach[$id]['SUMACH'] + 676;
$str1 = "SELECT NWB_ID, NWB_10digit,NWB_12digit,NWB_IDPROJ,TimeDownload,COUNT(NWB_Num) as COUNTACH,SUM(NWB_Num) as SUMACH ";
$str2 = " FROM tb_NWB WHERE NWB_STATUS LIKE 'Complete' and  NWB_10digit like '2204144301'  GROUP BY NWB_10digit";
$strqry = $str1 . " " . $str2;
$bantu01 = mysqli_query($conn, $strqry);
$bantu = [];
$row = [];
...
while ($row = mysqli_fetch_assoc($bantu01)) {
    $bantu1 = [];
    $bantu1['NWB_ID'] = '2204144301';
    $bantu1['NWB_10digit'] = $row['NWB_10digit'];
    $bantu1['NWB_12digit'] = '2204144301';
    $bantu1['NWB_IDPROJ'] = substr($row['NWB_IDPROJ'], 0, 14);
    $bantu1['TimeDownload'] = $row['TimeDownload'];
    $bantu1['COUNTACH'] = $row['COUNTACH'];
    $bantu1['SUMACH'] = $row['SUMACH'];
    $bantu[] = $bantu1;
    $ar_ach[] = $bantu1;
}
//#####################################################
...
////==========final 
$newarray = array_column($ar_ach, 'NWB_ID');
$strquery = "SELECT *,AVG(PROJ_Target) as Sum_Target, MIN(PROJ_Field_start) as minPROJ_Field_start,MAX(PROJ_Field_End) as maxPROJ_Field_End,MAX(PROJ_Field_End_Adj) as maxPROJ_Field_Endadj  FROM tb_Project_ownership WHERE PROJ_ID10DGT Not like '2204144301' and (PROJ_Status_DP=1 or PROJ_StatusFieldFinal = 1) GROUP BY PROJ_KODENWB";
$bantu01 = mysqli_query($conn, $strquery);
$SummPROJ = [];
$row = [];
$bantu = [];
while ($row = mysqli_fetch_assoc($bantu01)) {
    ...
    // if($id!=''&&strpos($row['PROJ_name_parent'],'Smoothie QT ')==''){
    if ($id != '') {
        if ((datedifference($skarang, $row['PROJ_Field_End_Adj']) >= 0) || $ar_ach[$id]['SUMACH'] <= $row['Sum_Target']) {
            $sqlstr = "UPDATE tb_Project_ownership set PROJ_StatusFieldFinal=1 WHERE PROJ_KODENWB='" . $row['PROJ_KODENWB'] . "'";
            $conn->query($sqlstr);
            $bantu['PROJ_StatusFieldFinal'] = 1;
        }
        if ((datedifference($skarang, $row['PROJ_Field_End_Adj']) < 0) && $ar_ach[$id]['SUMACH'] > $row['Sum_Target']) {
            $sqlstr = "UPDATE tb_Project_ownership set PROJ_StatusFieldFinal=0 WHERE PROJ_KODENWB='" . $row['PROJ_KODENWB'] . "'";
            $conn->query($sqlstr);
            $bantu['PROJ_StatusFieldFinal'] = 0;
        }
        if (strpos($row['PROJ_Project_name'], 'Smoothie QT W1') != '') {
            $sqlstr = "UPDATE tb_Project_ownership set PROJ_StatusFieldFinal=0 WHERE PROJ_KODENWB='" . $row['PROJ_KODENWB'] . "'";
            $conn->query($sqlstr);
            $sqlstr = "UPDATE tb_Project_ownership set PROJ_Status_DP=0 WHERE PROJ_KODENWB='" . $row['PROJ_KODENWB'] . "'";
            $conn->query($sqlstr);
            $bantu['PROJ_StatusFieldFinal'] = 0;
        }
        ...
        $bantu['PROJ_Field_End_Adj'] = $row['maxPROJ_Field_Endadj'];
        $bantu['lamafw'] = datedifference($bantu['PROJ_Field_start'], $bantu['PROJ_Field_End']);
...
//####################EDIT Buat SMOOTHIE################## 
$newarray = array_column($ar_ach, 'NWB_ID');
$strquery = "SELECT *,SUM(PROJ_Target) as Sum_Target, MIN(PROJ_Field_start) as minPROJ_Field_start,MAX(PROJ_Field_End) as maxPROJ_Field_End,MAX(PROJ_Field_End_Adj) as maxPROJ_Field_Endadj FROM tb_Project_ownership WHERE PROJ_ID10DGT LIKE '2204144301' and (PROJ_Status_DP=1 or PROJ_StatusFieldFinal = 1) GROUP BY PROJ_ID10DGT";
$bantu01 = mysqli_query($conn, $strquery);
// $SummPROJ_Smoothie = [];
while ($row = mysqli_fetch_assoc($bantu01)) {
    $bantu = [];
    $bantu['PROJ_StatusFieldFinal'] = $row['PROJ_StatusFieldFinal'];
    $bantu['PROJ_KODENWB'] = '2204144301';
    $bantu['PROJ_SFM'] = $row['PROJ_SFM'];
    $bantu['PROJ_IDGAB'] = $row['PROJ_ID10DGT'];
    $bantu['PROJ_ID10DGT'] = $row['PROJ_ID10DGT'];
    $bantu['PROJ_Target'] = $row['Sum_Target'];
    ...
    $bantu['lamafw'] = datedifference($bantu['PROJ_Field_start'], $bantu['PROJ_Field_End']);
    $bantu['PROJ_DP_End'] = $row['PROJ_DP_End'];
    $bantu['PROJ_Symphoni_num'] = '2204144301';
    $bantu['PROJ_Remark'] = $row['PROJ_Remark'];
    $bantu['PROJ_Project_name'] = substr($row['PROJ_Project_name'], 0, 14);
    $bantu['PROJ_name_parent'] = substr($row['PROJ_name_parent'], 0, 14);
    $bantu['PROJ_STATQC'] = $row['PROJ_STATQC'];
    $bantu['PROJ_Status_DP'] = $row['PROJ_Status_DP'];
    $SummPROJ[] = $bantu;
}
...
foreach ($SummPROJ as $row) {
    ...
    if ($row['PROJ_StatusFieldFinal'] == 1) {
        $bantu = [];
        $bantu['SummPROJ'] = $id;
        ...
        //$index = 0;
        if ((datedifference($row['PROJ_Field_start'], $skarang) + 0) <= 0) {
            $bantu['Status'] = '1. DP:Scripting';
        }
        if (((datedifference($row['PROJ_Field_start'], $skarang)) + 0) > 0 && ((datedifference($row['PROJ_Field_End_Adj'], $skarang)) + 0) <= 0) {
            $bantu['Status'] = '2. FW Stage';
        }
        if (((datedifference($row['PROJ_Field_End_Adj'], $skarang)) + 0) > 0) {
            $bantu['Status'] = '3. DP:Tabulation';
        }
        if ($row['PROJ_Field_start'] != '0000-00-00') {
            $lfw = datedifference($row['PROJ_Field_start'], $row['PROJ_Field_End']);
        } else {
            $lfw = '';
        }
        if ($lfw == 0 || $lfw == '') $lfw = 1;
        ...
        if (!($id5 == '')) {
            $bantu['TimeDownload'] = $SummNWB[$id5]['TimeDownload'];
        } else {
            $bantu['TimeDownload'] = '';
        }
        if (!($id2 == '')) {
        ...
        if (!($id4 == '')) {
            $bantu['PROJ_NAMEORIGINAL'] = $ar_ach[$id4]['NWB_ID'];
        } else {
            $bantu['PROJ_NAMEORIGINAL'] = '-';
        }
        ...
        if (!($id4 == '')) {
            $bantu['NWB_ACH'] = $ar_ach[$id4]['SUMACH'];
        } else {
            $bantu['NWB_ACH'] = '-';
        }
        $bantu['PROJ_DP_End'] = $row['PROJ_DP_End'];
        ...
        $bantu['now'] = $skarang;
        if ($row['PROJ_Field_start'] != '0000-00-00') {
            $bantu['PCT_TIME'] = round($lstarttonow / $row['lamafw'] * 100, 0);
        } {
            $bantu['PCT_TIME'] = '-';
        }
        $bantu['PCT_TIME'] = round($lstarttonow /  $row['lamafw'] * 100, 0);
        // if ($row['PROJ_Target'] > 0) $bantu['PCT_ACH'] = round($arr_varian[$id4]['NWB_MINACH'] / $row['PROJ_Target'] * 100, 0);
        if ($row['PROJ_Target'] > 0 && !($id4 == '')) {
            $bantu['PCT_ACH'] = round($ar_ach[$id4]['SUMACH'] / $row['PROJ_Target'] * 100, 0);
        } else {
            $bantu['PCT_ACH'] = 0;
        }
        if ($row['PROJ_Field_start'] != '0000-00-00') {
            $rasiotime = $lstarttonow /  $row['lamafw'];
        } {
            $rasiotime = '-';
        }
        if ($rasiotime != '-') {
            $hitrasio = $rasiotime * $row['PROJ_Target'];
        } else {
            $hitrasio = 0;
        }
        //if ($hitrasio > 0) $index = $arr_varian[$id4]['NWB_MINACH'] / $hitrasio;
        if (!($id5 == '') && !($id2 == '')) {
            $bantu['AVGLOAD'] = round($SummNWB[$id5]['SUMACH'] / $arr_intvload[$id2]['NWB_INT'], 0);
        } else {
            $bantu['AVGLOAD'] = 0;
        }
        ...
        if (!($id4 == '')) {
            if ($bantu['PCT_TIME'] != 0) $bantu['INDEXPCT'] = round($bantu['PCT_ACH'] / $bantu['PCT_TIME'] * 100, 0);
            if ($bantu['PCT_TIME'] == 0) $bantu['INDEXPCT'] = 100;
        } else {
            $bantu['INDEXPCT'] = 0;
        }
        if (!($id3 == '')) {
        ...
        $bantu['NWB_10digit'] = $row['PROJ_ID10DGT'];
        $bantu['NWB_12digit'] = $row['PROJ_Symphoni_num'];
...
<div id="div_refresh">
    ...
    <div class="main-content">
        <section class="section">
            ...
            <?php $session = session() ?>
            <h4>Selamat datang <?php echo $session->get('username') ?> (Anda sebagai User) - <strong>LIST OF ACTIVE PROJECTS - FIELD WORK</strong></h4>
            ...
            <div class="section-body">
                <div class="card">
                    ...
                    <div class="card-box table-responsive" width=40>
                        ...
                        <a href="<?= site_url('/home_user') ?>" class="btn btn-primary">
                            <i class=""></i> Active Project
                        </a>
                        <a href="<?= site_url('/home_user_script') ?>" class="btn btn-primary">
                            <i class=""></i> Scripting Process
                        </a>
                        <a href="<?= site_url('/home_user_tabulasi') ?>" class="btn btn-primary">
                            <i class=""></i> Tabulation Process
                        </a>
                        <a href="<?= site_url('/home_user_close') ?>" class="btn btn-primary">
                            <i class=""></i> Close Project
                        </a>
                        <a href="<?= site_url('/home_user_utility') ?>" class="btn btn-primary">
                            <i class=""></i> Utility
                        </a>
                        <a href="<?php echo base_url('/generatepdf') ?>" class="btn btn-primary">
                            ...
                            <thead>
                                <tr height=20 style="font-size:15px" ; bgcolor="dodgerblue">
                                    ...
                                    <th rowspan=2 class=xl124 width=80 style='width:80px'>
                                        <font color="white">FW-Start</font>
                                    </th>
                                    <th rowspan=2 class=xl124 width=80 style='width:80px'>
                                        <font color="white">FW-End<br>(*Adjusted)</font>
                                    </th>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr style="font-size:15px">
                                        <td bgcolor="white">
                                            <font color="black"><?= $ii++; ?></font>
                                        </td>
                                        <td bgcolor="white"><a href="/profile?NWB_INT=<?= $row['NWB_ID']; ?>">
                                                <font color="black"><?= substr($row['NWB_ID'], 0, 50); ?></font>
                                            </a></td>
                                        <td bgcolor="white"><a href="/profile?NWB_INT=<?= $row['NWB_ID']; ?>">
                                                <font color="black"><?= substr($row['NWB_12digit'], 0, 50); ?></font>
                                            </a></td>
                                        <td bgcolor="white"><a href="/profile?NWB_INT=<?= $row['NWB_ID']; ?>">
                                                <font color="black"><?= substr($row['PROJ_NAME'], 0, 50); ?></font>
                                            </a></td>
                                        <td bgcolor="white"><a href="/bydate?NWB_INT=<?= $row['NWB_ID'] . ';' . $row['NWB_ACH']; ?>">
                                                <font color="black"><?= $row['PROJ_Field_start']; ?></font>
                                            </a></td>
                                        <td bgcolor="white"><a href="/bydate?NWB_INT=<?= $row['NWB_ID'] . ';' . $row['NWB_ACH']; ?>">
                                                <font color="black"><?= $row['PROJ_Field_End_Adj']; ?></font>
                                            </a></td>
